...
const allMenu = [
    ...
    { id: 16, cat: 'minuman', name: 'Coffee Aren Velvet', price: 13000, img: 'img/Coffee Aren Velvet.jpg', unit: 'Cup', badge: '' },
];
            ...
            <div class="menu-card group bg-white rounded-[32px] overflow-hidden border border-gray-50 relative h-full flex flex-col">
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="mt-auto flex justify-between items-center">
                        <div>
                            <p class="text-pink-500 font-black text-xl">Rp ${item.price.toLocaleString()}</p>
        ...
        cartItems.innerHTML += `
            <div class="flex justify-between items-center bg-gray-50 p-5 rounded-[24px]">
                <div>
                    ...
                    <p class="text-pink-500 font-bold text-sm">${item.qty}x • Rp ${(item.price * item.qty).toLocaleString()}</p>
                </div>
...
function checkoutWhatsApp() {
    ...
    let text = "✨ *DIMSUM SULTAN ORDER* ✨\n\n";
    cart.forEach(item => text += `🔹 ${item.name} x${item.qty}\n`);
    text += `\n*TOTAL PEMBAYARAN: ${document.getElementById('total-price').innerText}*`;
